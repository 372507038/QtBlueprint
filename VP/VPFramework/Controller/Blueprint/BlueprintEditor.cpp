...
void BlueprintEditor::ReloadSKismetInspector()
{
	...
	if (m_SelType != EBlueprintItemType::None && !m_CurSelGuid.IsValid())
		return;
	...
	{
		UFunction* func = VPPOOL_FIND(m_Blueprint, m_CurSelGuid, class UFunction);
	...
	{
		UVariant* var = VPPOOL_FIND(m_Blueprint, m_CurSelGuid, class UVariant);
...
{
	if (m_CurSelGuid == guid)
	{
		m_SelType = EBlueprintItemType::None;
		m_CurSelGuid = m_CurDblGuid = FGuid();
	}
